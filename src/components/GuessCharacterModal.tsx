import { Avatar, Button, Modal, ModalBody, ModalCloseButton, ModalContent, ModalFooter, ModalHeader, ModalOverlay, Text, Menu, MenuButton, MenuList, MenuItem, Box, Flex, HStack } from "@chakra-ui/react";
...
const GuessCharacterModal: React.FC<GuessCharacterModalProps> = ({ isOpen, onClose, onGuess, remainingCharacters }) => {
                ...
                {remainingCharacters?.map((c, index) => (
                  <MenuItem key={index} onClick={() => setCharacter(c.name)}>
                    <Box display="flex" alignItems="center">
                      <Avatar name={c.name} src={`/${c.name}.png`} size="xs" mr="2" />
                      <Text>{c.name}</Text>
                    </Box>
                  </MenuItem>
                ))}
          ...
          {character && (
            <Flex justify="center" mt={4}>
              <HStack>
                ...
                <Text fontSize="2xl" ml="4">{character}</Text>
              </HStack>
            </Flex>
          )}
        </ModalBody>
